...
个人博客，部署在 GitHub Pages。**没有顶栏，也没有侧栏**——页面是居中的内容流。
导航收在页脚一行小链接里：**主页 / 归档 / 友链 / 关于**（外加 GitHub），
正文末尾另有「较新的文章 / 较旧的文章」。
...
- 宽屏（≥900px）：内容流约取整页 60% 宽；
- 小屏（<900px）：内容自动放宽到接近全宽，保证可读性；
- 想调内容宽度：改 `src/styles/global.css` 顶部的 `--measure-list`（以及
  `@media (max-width: 899px)` 里的回退宽度）。
