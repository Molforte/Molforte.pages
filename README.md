...
主导航是**应用式底栏**（固定在页面底部：主页 / 归档 / 友链 / 关于），
正文末尾另有「较新的文章 / 较旧的文章」。
...
- 宽屏（≥900px）：内容流约取整页 60% 宽，底栏标签与之对齐；
- 小屏（<900px）：内容放宽到接近全宽，底栏标签铺满整行；
- 想调内容宽度：改 `src/styles/global.css` 顶部的 `--measure-list`；
- 底栏样式（高度 / 毛玻璃 / 图标）：全局搜索 `apptabbar` 相关规则。
