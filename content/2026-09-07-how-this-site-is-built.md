...
date: 2026-09-07
tags: [技术, 前端]
summary: 这个博客不用 Hexo、Hugo 这类博客框架，也不用任何现成的博客主题。它是 React + Vite 的手工组合：文章是一篇篇放在仓库里的 Markdown 文件，剩下的交给几个很小的库。好处是结构完全透明、没有框架黑盒；代价是要自己处理一些框架早就替你处理好的事——路由、部署回退。文章全部放在 content/，一篇一个 Markdown 文件，文件名带日期和 slug；文件头部是一段 frontmatter，用自写的极简解析器读取。写文章就是往 content/ 里丢一个文件，构建时自动出现，没有数据库，没有后台。部署交给 GitHub Actions：推送到 main 后 npm ci && npm run build，再发布 dist 到 Pages。这套组合适合愿意自己维护、但不想碰框架黑盒的博客。
---
